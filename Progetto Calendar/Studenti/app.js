...
async function loadLocalCalendar() {
  try {
    ...
    if (!isLikelyXLSXArrayBuffer(buf)) throw new Error("Il file ottenuto non è un .xlsx valido (no firma PK)");
...
function autoDetectDateHeader(headers) {
  const chosen = dateColumnSelect && dateColumnSelect.value && dateColumnSelect.value !== "— nessuna —" ? dateColumnSelect.value : null;
...
function coveredMinutesWithinNeeds(intervals) {
  ...
  let total = 0;
  for (const need of needs) for (const iv of merged) {
    const start = Math.max(need.start, iv.start), end = Math.min(need.end, iv.end);
...
function fmtDateIT(d) {
  try {
    ...
    if (d instanceof Date) {
      ...
      const wd = days[d.getDay()]; const y = String(d.getFullYear()).slice(-2);
...
function fmtTimeFromFraction(fr) { const total = Math.round(fr*24*60); const hh=Math.floor(total/60), mm=total%60; return String(hh).padStart(2,"0")+":"+String(mm).padStart(2,"0"); }
...
function renderCourseButtons(year) {
  const area = $("#courseButtons"); const list = COURSES[String(year)] || [];
  area.innerHTML = list.map(c => `<button class="btn big primary" data-course="${c.key}">${c.label}</button>`).join("");
  $$("#courseButtons [data-course]").forEach((btn) => {
    btn.addEventListener("click", () => { goToCalendarWithCourse(btn.dataset.course); });
  });
}
...
function keyForRowPenultimate(row, dateH, startH, moduleH) {
  const mod = row[moduleH] ?? ""; const dateKey = dateKeyFromVal(row[dateH]) ?? "";
  const startMin = toMinutes(row[startH]); const t = startMin != null ? String(startMin) : "";
  return `${mod}__${dateKey}__${t}`;
}
function computePenultimateKeys(headers, rows) {
  penultimateKeys = new Set(); moduleHeaderName = autoDetectModuleHeader(headers);
  const dateH = autoDetectDateHeader(headers), startH = autoDetectStartHeader(headers);
  if (!moduleHeaderName || !dateH) return;
  const groups = new Map();
  for (const r of rows) {
    const mod = String(r[moduleHeaderName] ?? "").trim(); if (!mod) continue;
    const dVal = r[dateH]; const d = dVal instanceof Date ? dVal : new Date(dVal); if (isNaN(d)) continue;
    const sMin = startH ? toMinutes(r[startH]) : null;
    if (!groups.has(mod)) groups.set(mod, []);
    groups.get(mod).push({ row: r, d, sMin });
  }
  for (const [mod, arr] of groups.entries()) {
    arr.sort((a,b) => { const cmpD = a.d - b.d; if (cmpD !== 0) return cmpD; const aa=a.sMin ?? -1, bb=b.sMin ?? -1; return aa-bb; });
    if (arr.length >= 2) {
      const penult = arr[arr.length - 2];
      const k = keyForRowPenultimate(penult.row, dateH, startH, moduleHeaderName);
...
function computeDayCoverage(headers, rows) {
  ...
  coverageDateHeader = dateH; const map = new Map();
  rows.forEach((r) => {
...
function renderTable(_headersInput, rowsBase) {
  ...
  if (dateHeader && !showAll) {
    ...
    rows = rows.filter((r) => { const v = r[dateHeader]; const d = v instanceof Date ? v : new Date(v); if (isNaN(d)) return false; return d >= today; });
  }
  ...
  filtered.forEach((row) => {
    ...
    // evidenzia penultima
    { const dateH = autoDetectDateHeader(baseHeaders); const startH = autoDetectStartHeader(baseHeaders);
  ...
  if (_dateHeaderForSep) {
    ...
    createdRows2.forEach((tr, idx) => {
      const r = filtered[idx]; const key = dateKeyFromVal(r[_dateHeaderForSep]);
  ...
  if (coverageDateHeader) {
    ...
    createdRows.forEach((tr, idx) => {
      const r = filtered[idx]; const key = dateKeyFromVal(r[coverageDateHeader]); const minutes = dayCoverage.get(key) || 0;
...
async function handleFile(file) {
  ...
  if (pendingSheetName && workbook.SheetNames.includes(pendingSheetName)) defaultSheet = pendingSheetName;
  else defaultSheet = workbook.SheetNames.find((n) => n.toLowerCase().includes("fust")) || workbook.SheetNames[0];

  sheetSelect.value = defaultSheet; loadSheet(defaultSheet);
  setStatus(`Pronto: ${file.name}`, "ok"); fileInput.value = "";
}
function loadSheet(name) {
  const ws = workbook.Sheets[name];
  const { headers, rows } = excelToJson(ws);
  baseHeaders = headers.slice(); computeDayCoverage(baseHeaders, rows); computePenultimateKeys(baseHeaders, rows);
  allRows = rows.slice(); currentHeaders = baseHeaders.slice(); showAll = false; updateToggleButton();
  renderTable(baseHeaders, allRows); updateAddButtonLink();
}

// CLEAR
function clearAll() {
  tHead.innerHTML = ""; tBody.innerHTML = ""; rowsCount.textContent = "—";
  currentData = []; currentHeaders = []; baseHeaders = []; allRows = [];
  showAll = false; updateToggleButton();
  workbook = null; dayCoverage = new Map(); coverageDateHeader = null;
  sheetSelect.innerHTML = ""; dateColumnSelect.innerHTML = ""; timeColumnSelect.innerHTML = "";
  searchInput.value = ""; fileInput.value = ""; setStatus("Nessun file");
}
...
function goToCalendarWithCourse(courseKey) {
  ...
  courseSection.classList.add("hidden"); appSection.classList.remove("hidden"); btnBack.classList.remove("hidden");
  if (workbook && pendingSheetName && workbook.SheetNames.includes(pendingSheetName)) { sheetSelect.value = pendingSheetName; loadSheet(pendingSheetName); }
  const logo = document.getElementById("courseLogo"); if (logo) { logo.textContent = course.label; logo.classList.add("active-logo"); }
}
...
$("#btnBack")?.addEventListener("click", () => {
  appSection.classList.add("hidden"); courseSection.classList.remove("hidden"); btnBack.classList.add("hidden");
...
$("#btnToggleAll")?.addEventListener("click", () => { showAll = !showAll; updateToggleButton(); renderTable(baseHeaders, allRows); });
...
function getCurrentSheetCalendarId() {
  ...
  const sheetName = sheetSelect.value; return CALENDAR_BY_SHEET[sheetName] || null;
}
...
$("#btnPushEvents")?.addEventListener("click", () => {
  const calId = getCurrentSheetCalendarId();
  if (!calId) return setStatus("Seleziona un corso/foglio con calendario associato.", "err");
...
function updateAddButtonLink() {
  const btn = $("#btnPushEvents"); if (!btn) return;
  const calId = getCurrentSheetCalendarId();
  if (calId) {
    btn.disabled = false; btn.dataset.calId = calId;
    btn.textContent = "➕ Abbonati a questo calendario";
    btn.title = "Apri Google Calendar e aggiungi il calendario del corso attivo";
  } else {
    btn.disabled = true; delete btn.dataset.calId;
    btn.textContent = "➕ Abbonati a questo calendario";
  }
}
